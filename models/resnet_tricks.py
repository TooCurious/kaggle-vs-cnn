...
class BaseBlock(nn.Module):
    ...
    At each repeat the block has two paths for the input:
        ...
        - Path B: Identity or (Conv -> BN) or (AvgPool -> Conv -> BN)
                  A shortcut connection is defined as follows:
                      1. 'Downsampling' layer that consists of (depending on 'trick_downsample' parameter):
                            a) one convolution with the same depth as those from path A, kernel = (1, 1) and stride = 2
                            b) or Avg Pool layer with kernel = (2, 2) and stride = 2 and convolution with the same depth
                                    as those from path A, kernel = (1, 1) and stride = 1
    ...
    def __init__(self, in_channels: int, block_out_channels: int, stride: int, expansion: int = 1,
                 trick_downsample: bool = True, zero_gamma: bool = True):
        """
        Args:
            ...
            trick_downsample: If to use tricked 'Downsampling' layer.
            zero_gamma: If to mark last BN layer to be zeroed while parameters initialization.
        """
        ...
        if in_channels == block_out_channels * expansion:
            ...
            if trick_downsample:
                self.shortcut = nn.Sequential(
                    ...
                    nn.Conv2d(in_channels, block_out_channels * expansion, kernel_size=1, stride=1, bias=False),
    ...
    def set_last_bn(self):
        """Adds 'is_last' attribute to the last Batch Norm at residual block."""
        last_bn = None
...
class BaseBlockFullPreAct(nn.Module):
    """A base block used as a building block for ResNet-18/34 models.

    This block is usually repeated several times within each of the model stages.

    At each repeat the block has two paths for the input:
        - Path A: BN -> ReLU -> Conv -> BN -> ReLU -> Conv (skipping first BN and ReLU when there is a stem before)
                  Two convolutions defined as follows:
                      1. The same depth for both convolutions (depth is specified in model_cfg.resnet.layers for each stage)
                      2. The same kernel size = (3, 3) for both convolutions
                      3. The same padding = 1 for both convolutions
    ...
    def __init__(self, in_channels: int, block_out_channels: int, stride: int, expansion: int = 1,
                 after_stem: bool = False, trick_downsample: bool = True):
        """
        Args:
            in_channels: The number of input channels.
            block_out_channels: The number of output channels for the first block,
            ...
            trick_downsample: If to use tricked 'Downsampling' layer.
        """
        super(BaseBlockFullPreAct, self).__init__()
        if in_channels == block_out_channels * expansion:
            self.shortcut = nn.Identity()
        else:
            if trick_downsample and stride != 1:
                self.shortcut = nn.Sequential(
                    nn.AvgPool2d(kernel_size=2, stride=stride),
                    nn.Conv2d(in_channels, block_out_channels * expansion, kernel_size=1, stride=1, bias=False)
                )
                ...
                self.shortcut = nn.Conv2d(
                    in_channels, block_out_channels * expansion, kernel_size=1, stride=stride, bias=False
                )
...
class BottleneckBlock(nn.Module):
    ...
    At each repeat the block has two paths for the input:
        ...
        - Path B: Identity or (Conv -> BN) or (AvgPool -> Conv -> BN)
                  A shortcut connection is defined as follows:
                      1. 'Downsampling' layer that consists of (depending on 'trick_downsample' parameter):
                            a) one convolution with the same depth as for the last convolution from path A,
                                    kernel = (1, 1) and stride = 2
                            b) or Avg Pool layer with kernel = (2, 2) and stride = 2 and convolution with the same depth
    ...
    def __init__(self, in_channels: int, block_out_channels: int, stride: int, expansion: int = 4,
                 trick_downsample: bool = True, zero_gamma: bool = True):
        """
        Args:
            in_channels: The number of input channels.
       ...
       """
        ...
        if in_channels == block_out_channels * expansion:
            ...
            if trick_downsample and stride == 2:
                self.shortcut = nn.Sequential(
                    nn.AvgPool2d(kernel_size=2, stride=stride),
                    nn.Conv2d(in_channels, block_out_channels * expansion, kernel_size=1, stride=1, bias=False),
                    nn.BatchNorm2d(block_out_channels * expansion)
...
class BottleneckBlockFullPreAct(nn.Module):
    """A 'bottleneck' block used as a building block for ResNet-50/101/152
    ...
    def __init__(self, in_channels: int, block_channels: int, stride: int, expansion: int = 4, after_stem=False,
                 trick_downsample: bool = True):
        """
        Args:
            in_channels: The number of input channels.
            block_channels: The number of output channels for the first two blocks, number of output channels
                    for the third convolution and the whole building block is block_channels * expansion.
            stride: Stride for the second convolution in path A and convolution in path B (in 'Downsampling' case).
            expansion: The factor by which block_channels is multiplied to get the whole building block output channels number.
            after_stem: If this block follows the stem block.
            trick_downsample: If to use tricked 'Downsampling' layer.
        """
        super(BottleneckBlockFullPreAct, self).__init__()
        if in_channels == block_channels * expansion:
            self.shortcut = nn.Identity()
        else:
            if trick_downsample and stride == 2:
                self.shortcut = nn.Sequential(
                    nn.AvgPool2d(kernel_size=2, stride=stride),
                    nn.Conv2d(in_channels, block_channels * expansion, kernel_size=1, stride=1, bias=False),
                )
            else:
                self.shortcut = nn.Conv2d(
                    in_channels, block_channels * expansion, kernel_size=1, stride=stride, bias=False
                )

        if after_stem:
            self.conv_block = nn.Sequential(
                nn.Conv2d(in_channels, block_channels, kernel_size=1, stride=1, bias=False),
                nn.BatchNorm2d(block_channels),
                nn.ReLU(),
                nn.Conv2d(block_channels, block_channels, kernel_size=3, stride=stride, padding=1, bias=False),
                nn.BatchNorm2d(block_channels),
                nn.ReLU(),
                nn.Conv2d(block_channels, block_channels * expansion, kernel_size=1, stride=1, bias=False),
            )
        else:
            self.conv_block = nn.Sequential(
                nn.BatchNorm2d(in_channels),
...
class ResNetTricks(nn.Module):
    """ResNet model with tricks."""

    def __init__(self, config, in_channels: int = 3, classes_num: int = 200):
        super(ResNetTricks, self).__init__()
        self.config = config
    ...
    def _make_stem(self, in_channels: int) -> (list, int):
        ...
        Scheme: Conv -> BN -> ReLU -> MaxPool or tricked version:
                Conv -> BN -> ReLU -> Conv -> BN -> ReLU -> Conv -> BN -> ReLU -> MaxPool

        Args:
            in_channels: The number of input channels.
        """
        if self.config.resnet_tricks.stem is not None:
            out_channels = self.config.resnet_tricks.stem
            if self.config.resnet_tricks.trick_stem:
                stem = [
    ...
    def _make_stages(self, previous_output: int) -> (list, int):
        ...
        """
        stages, expansion = [], self.config.resnet_tricks.expansion_factor

        for stage in self.config.resnet_tricks.layers['stages']:
        ...
        if self.config.resnet_tricks.pre_act:
            stages.extend([nn.BatchNorm2d(previous_output), nn.ReLU()])
